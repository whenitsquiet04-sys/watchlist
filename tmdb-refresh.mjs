...
const seasonFromTitle = (t) => { const m = String(t || "").match(/season\s+(\d+)/i) || String(t || "").match(/\bs(\d+)\b/i); return m ? Number(m[1]) : null; };
const yearOf = (item) => {
...
async function scheduleForTv(tv) {
  ...
  if (eps.length < 2) return { season: seasonNo, episodes: total, cadence: "tba", firstAir: eps[0]?.air_date };
  const gaps = [];
  ...
  return { season: seasonNo, cadence, episodes: total, endDate: finaleDate, firstAir: eps[0].air_date };
}
...
async function refreshTv(item) {
  ...
  // date: advance a returning show to its newest season premiere from TMDB, but
  // never move a curated date backwards; fill an empty date outright.
  if (sched.firstAir && (out.date == null || sched.firstAir > out.date)) { out.date = sched.firstAir; if (out.tbaLabel) delete out.tbaLabel; }
  if (out.endDate == null && sched.endDate) out.endDate = sched.endDate;
  // season number for labelling (an explicit "Season N" in the title wins)
  const sn = seasonFromTitle(out.title) || (sched.season && sched.season >= 2 ? sched.season : null);
  if (sn) out.season = sn;
        ...
        try {
          ...
          for (const r of (res.results || [])) {
            ...
            try {
              ...
              const date = (kind === "tv" && sched.firstAir) ? sched.firstAir : (data.release_date || data.first_air_date || null);
              const item = {
              ...
              if (kind === "tv" && sched.season && sched.season >= 2) item.season = sched.season;
